...
Usage:
    TDT_CC.py family <family_Relations_File> [options]
    TDT_CC.py doTDT <vcf_File> <family_Relations_File> <outputFile_Name> <cores> <chunksize> [options]
    TDT_CC.py doCaseControl <vcf_File> <family_Relations_File> <outputFile_Name> <cores> <chunksize> [options]
...
def doTDT(v, family, AC_Thresh, GQ_Kid_Thresh, GQ_Parent_Thresh, DP_Thresh, 
    ...
    # we are calling str on some of the variables because they are NONE which gives us a type error
    line = [v.get('CHROM'), v.get('POS'), str(v.get('SNPEFF_GENE_NAME')),
            str(v.get('SNPEFF_EFFECT')), str(v.get('SNPEFF_FUNCTIONAL_CLASS')),
            v.get('AC'), str(TU[0]), str(TU[1])]
...
def doCaseControl(v, cases, controls, AC_Thresh, GQ_CC_Thresh, DP_Thresh, 
                  PL_Thresh, synFlag):
    ...
    # loop through all the individuals in the case dict        
    ...
    #loop through all indivs in the control dict
    ...
    # we are calling str() on some of the variables
    # because they are NONE which gives us a type error
    line = [v.get('CHROM'), v.get('POS'), str(v.get('SNPEFF_GENE_NAME')),
            str(v.get('SNPEFF_EFFECT')), str(v.get('SNPEFF_FUNCTIONAL_CLASS')),
            v.get('AC'), str(caseRefs), str(caseAlts),
            str(controlRefs), str(controlAlts)]
...
if __name__ == "__main__":
    ...
    if args['family']:
        ...
        writer.writerow(["CHROM,POSITION,GENE_NAME,SNPEFF_EFFECT",
                         "SNPEFF_FUNCTIONAL_CLASS,AC,transmitted,untransmitted"])  
        ...
        # Note: as it is currently, you are storing the results of lines that aren't processed,
        # it would be better if you could figure out a way to not even store those results to save on memory usage
        for result in map(partial_TDT, VCF.lines(args['<vcf_File>'])):
        ...
        # write out the header
        writer.writerow(["CHROM,POSITION,GENE_NAME,SNPEFF_EFFECT,SNPEFF_FUNCTIONAL_CLASS,AC",
